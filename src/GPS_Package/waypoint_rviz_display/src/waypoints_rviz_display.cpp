...
void init_waypoint(void)
{
	FILE *fp;
	int result = -10;
	fp= fopen("/home/adg/catkin_ws/src/GPS_Package/GPS_GUI/scripts/automative_car_test_course","r"); 
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher markerTXT_pub = n.advertise<visualization_msgs::MarkerArray>("marker_txt/node", 1);
  ...
  visualization_msgs::MarkerArray node_arr;
   visualization_msgs::MarkerArray node_TXT;
  ...
  for (size_t i = 0; i < vec_point.size(); i++)
  {
        ...
        node_TXT.markers.push_back(node_txt);
  } 
    ...
    if(use_gps_init_datum == 0) 
    {
	  ...
	  // printf("use topic datum initialization %8.4lf %8.4lf %8.4lf %8.4lf \n\n", datum_lat, datum_lon, datum_utm_east, datum_utm_north);  
       ...
       for(int i=0; i<no_waypoints; i++)
       {
        ...
        node_arr.markers[i].color.a = 1.0; 
        node_TXT.markers[i].color.g = 0.5;
			  node_TXT.markers[i].color.b = 0.0;
        node_TXT.markers[i].color.a = 1.0; 
        ...
        //  printf("%d %lf %lf %lf %lf \n",i,my_waypoints_list[i].x, my_waypoints_list[i].y, datum_utm_east, datum_utm_north);
          ...
          node_TXT.markers[i].pose.position.x = p.x;
          node_TXT.markers[i].pose.position.y = p.y;
          
	   }
    	
	}	  
  // ROS_INFO("marker_array.markers.size() Number %zu", node_arr.markers.size());
	marker_pub .publish(node_arr);  
  markerTXT_pub.publish(node_TXT);
